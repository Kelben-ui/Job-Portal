<?php
   require_once 'D:/Xampp/htdocs/JobPortal/app/controllers/OfferController.php';
   ...
   if($uri === '/JobPortal/app/api/offers.php' && $method === 'GET')
   {
   ...
   else if ($uri === '/JobPortal/app/api/offers.php' && $method === 'POST')
   {
      $data = json_decode(file_get_contents('php://input'), true);
      $controller->create_offer($data);
   }
   else if(preg_match('#^/JobPortal/app/api/offers.php/(job|internship)/([^/]+)$#', $uri,$matches) && $method === 'PUT')
   {
     $data = json_decode(file_get_contents('php://input'), true);
     $type = $matches[2];
   ...
   else if(preg_match('#^/JobPortal/app/api/offers.php/(job|internship)/([^/]+)$#', $uri, $matches) && $method === 'DELETE')
   {
     $type = $matches[2];
